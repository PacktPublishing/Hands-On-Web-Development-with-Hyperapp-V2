...
export const toggleCollapseComment = (state, { id }) => {
  const isCollapsed = state.story.collapsedComments.some(commentId => commentId === id);

  return {
    ...state,
    story: {
      ...state.story,
      collapsedComments: isCollapsed
        ? state.story.collapsedComments.filter(commentId => commentId !== id)
        : state.story.collapsedComments.concat(id),
    },
  };
};
