...
const CommentData = (comment, replies, now, collapsed) => {
  const article = collapsed
    ? []
    : (comment.deleted
        ? ['deleted']
    ...
    h('attribution', { class: '--small --light' }, [
      ...
      h(
        'button',
        {
          class: '--link',
          type: 'button',
          onclick: [actions.toggleCollapseComment, { id: comment.id }],
        },
        collapsed ? '[+]' : '[-]',
      ),
    ]),
...
const Comment = (comment, replies, now, collapsedComments) => {
  return comment && comment.time
    ? CommentData(comment, replies, now, (collapsedComments || []).indexOf(comment.id) >= 0)
    : CommentShadow();
}
...
const Thread = (comment, allComments, now, collapsedComments) => h('div', null, [
  Comment(comment, (comment && comment.kids || []).map(id => {
    const child = allComments.find(c => c.id === id);
    return child && Thread(child, allComments, now, collapsedComments);
  }), now, collapsedComments),
]);

const Discussion = ({ item, comments, collapsedComments }, now) => h('div', null, [
  h('storyRoot', null, [
    Story(item, now, true),
    h('hr'),
  ]),
  (item.kids || []).map(id => {
    ...
    return comment && Thread(comment, comments, now, collapsedComments);
  }),
]);
