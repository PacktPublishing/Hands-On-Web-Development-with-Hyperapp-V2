...
const bootstrapPopovers = () => {
  const observeElement = document.getElementById('app');
  const query = '[data-toggle="popover"]';

  const anyAddedNodes = (list, callback) => list.some(item =>
    Array.from(item.addedNodes).some(callback)
  );
  const isPopover = node => node.matches && node.matches(query);
  const doesNodeHavePopovers = node => node.querySelector && !!node.querySelector(query)

  const observer = new MutationObserver((mutationList) => {
    const anyNewPopovers = anyAddedNodes(mutationList, (node) => (
      isPopover(node) || doesNodeHavePopovers(node)
    ));

    if (anyNewPopovers) {
      $(query, observeElement).popover();
    }
  });
...
export const bootstrapPopoversSub = () => [bootstrapPopovers, {}];
