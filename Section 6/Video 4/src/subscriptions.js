...
/*
 * Get changed data from web component
 */

const webComponentAttrChange = (dispatch, { emitter, action }) => {
  const cancel = emitter.add((attr, prev, value) => {
    dispatch(action, { params: { type: value } });
  });
  return () => {
    cancel();
  };
...
export const webComponentAttrChangeSub = ({ action, emitter }) => [
  webComponentAttrChange,
  {
    action,
    emitter,
  },
